...
let tests =
    testList "Changeset"
        [ testCase "getQuery" <| fun _ ->
          ...
          testCase "insertQuery"
          <| fun _ ->
              let query = Queries.insertQuery<User>()
              printfn "%s" query
              Expect.isTrue (query = "insert into users (age, name) values (@age, @name) returning *")
                  "Insert generated"

          testCase "updateQuery"
          <| fun _ ->
              let query = Queries.updateQuery<User>()
              Expect.isTrue (query = "update users set (age = @age, name = @name) where user_id = @user_id")
                  "Update generated"
