...
module Changeset =
    ...
    let validate changeset =
        let foldFunc (valid, errors) { func = func; field = FieldName fieldName; errMsg = ErrMessage errMsg } =
            if func changeset.data then (false, Map.add fieldName errMsg errors)
            else (valid, errors)

        let (valid, errors) = changeset.validations |> List.fold foldFunc (true, Map.empty)
